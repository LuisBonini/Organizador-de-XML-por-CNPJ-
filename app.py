# app.py (Versão 3.3.2 - Correção do KeyError)
...
# --- 1. Funções de Cadastro (Leitura/Escrita do JSON) ---
...
# --- 2. Funções de Lógica Fiscal (O "Motor") ---
...
def auto_ajustar_colunas(writer, df, nome_aba):
    workbook = writer.book
    worksheet = writer.sheets[nome_aba]
    cell_format = workbook.add_format({'font_name': 'Arial', 'font_size': 10})

    for i, col in enumerate(df.columns):
        tamanho_coluna = max(len(str(col)), df[col].astype(str).str.len().max())
        tamanho_coluna = min(tamanho_coluna, 60) 
        tamanho_coluna = max(tamanho_coluna, 8)  
        worksheet.set_column(i, i, tamanho_coluna, cell_format)
    
    if df.index.name is not None:
        tamanho_indice = max(len(str(df.index.name)), df.index.astype(str).str.len().max())
        tamanho_indice = min(tamanho_indice, 60)
        tamanho_indice = max(tamanho_indice, 10)
        worksheet.set_column(0, 0, tamanho_indice, cell_format)
...
def criar_excel_in_memory(dfs_para_salvar):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        for nome_aba, df in dfs_para_salvar.items():
            ...
            auto_ajustar_colunas(writer, df, nome_aba)
...
# --- 3. A APLICAÇÃO STREAMLIT (Interface) ---
...
# Carregar CNPJs para o estado da sessão
...
# --- Aba 1: Processar XMLs (Tela Principal) ---
with tab_processar:
    st.header("1. Faça o Upload dos XMLs")
    
    if not st.session_state.cnpjs:
        st.error("Nenhum CNPJ cadastrado. Por favor, vá para a aba '⚙️ Configurar CNPJs' e adicione pelo menos um.")
    else:
        ...
        if st.button("Processar XMLs", type="primary", disabled=(not xml_files)):
            with st.spinner("Processando... Isso pode levar um tempo."):
                ...
                st.success(f"Processamento Concluído! {total_xmls_processados} de {total_xmls} XMLs eram relevantes e continham {total_itens_processados} itens.")
                
                if not dados_por_cnpj:
                    st.warning("Nenhuma nota fiscal válida para os CNPJs informados foi encontrada nos arquivos.")
                    st.session_state.relatorios = [] # Limpa relatórios antigos
                else:
                    relatorios_gerados = []
                    for cnpj_empresa, todos_os_itens in dados_por_cnpj.items():
                        ...
                        agregacoes = {
                            'Tipo': 'first', 'Nome Emitente': 'first', 'Nome Destinatario': 'first', # <-- CORREÇÃO 2
        ...
        # --- Seção de Download (Fora do "if st.button") ---
...
# --- Aba 2: Configurar CNPJs (Tela Secária) ---
with tab_config:
    st.header("Gerenciar CNPJs da Sua Empresa")
    st.info("Cadastre aqui todos os CNPJs que você considera como 'seus'. O script irá separar os relatórios com base nesta lista.")
        ...
        if st.button("Adicionar CNPJ", key="add_cnpj_btn"):
            ...
            if cnpj_limpo and cnpj_limpo.isdigit() and len(cnpj_limpo) == 14:
                if cnpj_limpo not in st.session_state.cnpjs:
                    ...
                    st.success(f"CNPJ {cnpj_limpo} adicionado com sucesso!")
                    st.rerun() 
                else:
                    st.warning("Este CNPJ já está cadastrado.")
            else:
                st.error("CNPJ inválido. Digite apenas os 14 números.")
    ...
    with st.expander("Remover CNPJs Cadastrados", expanded=True):
        if not st.session_state.cnpjs:
            ...
            with st.form("form_remocao"):
                cnpj_para_remover = st.selectbox("Selecione um CNPJ para remover:", st.session_state.cnpjs)
